...
namespace potent {
	...
	private:
		...
		std::uint32_t mPostProcessFlags = 0;
	...
	public:
		...
		void setFlag(std::uint32_t postProcessFlags) {
			mPostProcessFlags = postProcessFlags;
		}

		std::uint32_t getFlags() { return mPostProcessFlags; }
		...
		void render(RenderData* pRenderData, RMat projectionMatrix, RMat viewMatrix, RMat transformMatrix = RMat::Identity()) {
			...
			/*if (mPostProcessFlags != 0) {
				glUniform1i(glGetUniformLocation(mShaderProgram.id, "uPostProcessingFlags"), mPostProcessFlags);
			}*/
	...
	void debugDrawModelLines(RenderObjectData* model, RMat projection, RMat view, bool transposeTransform = 1) {
		Shader vs, fs;
		vs.loadFromMemory(DEBUG_DRAW_VERTEX_SHADER, GL_VERTEX_SHADER);
		fs.loadFromMemory(DEBUG_DRAW_FRAGMNET_SHADER, GL_FRAGMENT_SHADER);
		ShaderProgram sp;
	...
	void debugDrawLinesBox(RMat projection, RMat view, RMat transform = RMat::Identity()) {
		std::vector<float> p = {
			1.0f, 1.0f, 1.0f,
			-1.0f, 1.0f, 1.0f,
	...
	void debugDrawCollisionMesh(CollisionMesh* pCollisionMesh, RMat projection, RMat view, int type = GL_LINES) {
		Shader vs, fs;
		vs.loadFromMemory(DEBUG_DRAW_VERTEX_SHADER, GL_VERTEX_SHADER);
		fs.loadFromMemory(DEBUG_DRAW_FRAGMNET_SHADER, GL_FRAGMENT_SHADER);
		ShaderProgram sp;
